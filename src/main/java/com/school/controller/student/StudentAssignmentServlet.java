    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (student != null && student.getClass_id() > 0) {
                String sql = "SELECT a.*, t.name AS teacher_name FROM st_assignment a "
                           + "LEFT JOIN st_teacher t ON a.teacher_id = t.teacher_id "
                ...
                while (rs.next()) {
                    ...
                    a.put("teacher_name",  rs.getString("teacher_name") != null ? rs.getString("teacher_name") : "-");
                    assignments.add(a);
                }
            }
            ...
            StudentExceptionHandler.handle(request, response,
                new StudentAssignmentException("We could not load your assignments. Please try again later.", e));
            return;
        }
        request.getRequestDispatcher("/jsp/student/assignments.jsp").forward(request, response);
    }
}