        ...
        try {
            ...
            if (student.getClass_id() > 0) {
                ...
                PreparedStatement tps = con.prepareStatement(
                    "SELECT DISTINCT subject FROM st_timetable WHERE class_id=?");
                tps.setInt(1, student.getClass_id());
                ResultSet trs = tps.executeQuery();
                StringBuilder sb = new StringBuilder();
                while (trs.next()) {
                    if (sb.length() > 0) sb.append(",");
                    sb.append(trs.getString("subject"));
                }
            ...
            PreparedStatement attPs = con.prepareStatement(
                ...
                + "FROM st_attendance WHERE student_id=?");
            attPs.setInt(1, student.getStudent_id());
            ResultSet attRs = attPs.executeQuery();
            if (attRs.next()) {
                attTotal   = attRs.getInt("total");
                attPresent = attRs.getInt("present_count");
            }
            ...
            PreparedStatement examPs = con.prepareStatement(
                "SELECT subject, exam_date FROM st_exam WHERE class_id=? AND exam_date >= SYSDATE "
                + "ORDER BY exam_date FETCH FIRST 1 ROW ONLY");
            ...
            PreparedStatement aPs = con.prepareStatement(
                "SELECT * FROM st_assignment WHERE class_id=? ORDER BY upload_date DESC FETCH FIRST 5 ROWS ONLY");
            ...
            PreparedStatement ttPs = con.prepareStatement(
                "SELECT * FROM st_timetable WHERE class_id=? AND day=? ORDER BY start_time");
            ...
            PreparedStatement notiPs = con.prepareStatement(
            ...
            PreparedStatement rPs = con.prepareStatement(
                "SELECT r.*, e.total_marks AS max_marks FROM st_result r "
                + "LEFT JOIN st_exam e ON r.exam_id = e.exam_id "
                + "WHERE r.student_id=? ORDER BY r.result_id DESC FETCH FIRST 3 ROWS ONLY");
            ...
            StudentExceptionHandler.handle(request, response,
                new StudentDashboardException("We could not load your dashboard. Please try again later.", e));
            return;
        }
