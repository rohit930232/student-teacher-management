    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (student != null && student.getClass_id() > 0) {
                String sql = "SELECT oc.*, t.name AS teacher_name FROM st_online_class oc "
                           + "LEFT JOIN st_teacher t ON oc.teacher_id = t.teacher_id "
                           + "WHERE oc.class_id=? ORDER BY oc.start_time";
                ...
                while (rs.next()) {
                    ...
                    c.put("start_time",   rs.getString("start_time")   != null ? rs.getString("start_time")   : "");
                    c.put("end_time",     rs.getString("end_time")     != null ? rs.getString("end_time")     : "");
                    c.put("teacher_name", rs.getString("teacher_name") != null ? rs.getString("teacher_name") : "-");
                    c.put("status",       rs.getString("status")       != null ? rs.getString("status")       : "");
                    if ("Live".equals(rs.getString("status"))) liveClasses.add(c);
            ...
            StudentExceptionHandler.handle(request, response,
                new StudentOnlineClassException("We could not load online classes. Please try again later.", e));
            return;
        }
        request.getRequestDispatcher("/jsp/student/onlineclass.jsp").forward(request, response);
    }
}