    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (student != null && student.getClass_id() > 0) {
                String sql = "SELECT n.*, t.name AS teacher_name FROM st_notes n LEFT JOIN st_teacher t ON n.teacher_id=t.teacher_id WHERE n.class_id=? ORDER BY n.created_date DESC";
        	...
        	StudentExceptionHandler.handle(request, response,
                new StudentNotesException("We could not load your study notes. Please try again after some time.", e));
        return;
        }
        RequestDispatcher rd = request.getRequestDispatcher("/jsp/student/notes.jsp");
