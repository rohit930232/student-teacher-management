...
public class StudentResultServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (student != null) {
                String sql = "SELECT r.*, e.total_marks AS max_marks, e.exam_date FROM st_result r LEFT JOIN st_exam e ON r.exam_id=e.exam_id WHERE r.student_id=? ORDER BY r.result_id DESC";
        	...
        	StudentExceptionHandler.handle(request, response,
        	        new StudentResultException("We could not load your result details. Please try again later.", e));
        	    return;
        	}
        RequestDispatcher rd = request.getRequestDispatcher("/jsp/student/result.jsp");
