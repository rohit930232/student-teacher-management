...
@WebServlet("/admin/attendance/staff/mark")
public class AdminStaffAttMarkServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res)
        ...
        try {
            ...
            for (int i = 0; i < records.length(); i++) {
                ...
                int sid        = Integer.parseInt(rec.getString("id"));
                String status  = rec.getString("status");
                if (!dao.markStaffAttendance(sid, date, status)) allOk = false;
            }
